...
def test_local_files_present(local_registry: RegistryTest):
    with local_registry:
        _param_files = get(
            "http://127.0.0.1:8000/api/",
            "data_product",
        ...
        _param_file_obj = url_get(
            _param_files[0]["object"], local_registry._token
        )
        _store = url_get(
            _param_file_obj["storage_location"], local_registry._token
        )
        _path = _store["path"]
    ...
    with _cli_runner.isolated_filesystem(pyDataPipeline):
        with remote_registry, local_registry:
            ...
            _res = _cli_runner.invoke(
                cli, ["run", _new_cfg_path, "--debug", "--dirty"]
            )
        ...
        with remote_registry, local_registry:
            ...
            with open(
                ...
                yaml.dump(
                    {
                        "data_product": {
                            "testing:SEIRS_model/results/figure/python@v0.0.1": False
                        },
                        "file": {},
                        "job": {},
                    },
            ...
            fdp_serv.update_registry_post_setup(pyDataPipeline, True)
            with open(
                os.path.join(pyDataPipeline, FAIR_FOLDER, "staging")
            ) as cfg:
                _staging = yaml.safe_load(cfg)
            assert (
                "testing:SEIRS_model/results/figure/python@v0.0.1"
                in _staging["data_product"]
            )
            mocker.patch(
                "fair.configuration.get_local_data_store",
                lambda *args: os.path.join(local_registry._install, "data"),
            )
            with capsys.disabled():
                print(
                    "\tRUNNING: fair add testing:SEIRS_model/results/figure/python@v0.0.1"
                )
            ...
            _res = _cli_runner.invoke(
                cli,
                ["add", "testing:SEIRS_model/results/figure/python@v0.0.1"],
            )
