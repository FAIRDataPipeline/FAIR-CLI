...
def test_registry_entry_for_file(stager: fdp_stage.Stager, mocker: pytest_mock.MockerFixture):
    ...
    def dummy_get(uri, obj_path, params):
        ...
        return [_url]
    mocker.patch('fair.registry.requests.get', lambda *args, **kwargs: dummy_get(*args, **kwargs))
    assert stager.find_registry_entry_for_file(LOCAL_REGISTRY_URL, '/not/a/path') == _url


@pytest.mark.staging
def test_get_job_data(local_registry, stager: fdp_stage.Stager, local_config: typing.Tuple[str, str], mocker: pytest_mock.MockerFixture):
    with local_registry:
